                          ...
                          HPDF_Encrypt e)
{
	...
	printf("xref write to stream , stream size = %d\n" , stream->size);
    ...
    while (tmp_xref) {
        ...
        for (i = str_idx; i < tmp_xref->entries->count; i++) {
            ...
            *pbuf++ = ' ';
            ...
            printf("i=%d xred wola write to stream stream size=%d buf size = %d\n",i, stream->size, strlen(buf));
    ...
    while (tmp_xref) {
	    printf("write cross references table %d \n" , stream->size);
