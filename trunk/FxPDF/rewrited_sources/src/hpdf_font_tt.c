                  ...
                  HPDF_Xref        xref)
{
	...
	printf("HPDF_TTFONT_NEW encoder->type = %d \n", encoder->type);
     ...
     * for the first time.
     */
	 printf("linia 121 sizeof(INT16)= %d " , sizeof(HPDF_INT16) * 256);
	...
	printf("linia 131 sizeof(INT16)= %d " , sizeof(HPDF_INT16) * 256);
           ...
           HPDF_BYTE  code)
{
    ...
    printf("HPDF_FONT_TT char width char = %d \n" , code);

    if (attr->used[code] == 0) {
	 
        HPDF_UNICODE unicode = HPDF_Encoder_ToUnicode (attr->encoder, code);
		printf("used bylo 0 unicode = %X \n", unicode);
        attr->used[code] = 1;
            ...
            HPDF_UINT         len)
{
   ...
   printf("HPDF_FONT_TT - TextWidth , text = %s \n" , text );
