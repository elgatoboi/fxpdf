...
printf("pend_count = %d\n", *pend_count);
    ...
    if (attr->cmap.id_range_offset[i] == 0) {
		...
		printf(" HPDF_TTFontDef_GetGlyphid idx=%u code=%u "
							" ret=%u\n", i, unicode,
							unicode + attr->cmap.id_delta[i]);
        ...
        printf(" calculated idx =  %d id_range_offset =   %d   start count = %d  seg count  =%d \n " ,idx,  attr->cmap.id_range_offset[i] , attr->cmap.start_count[i] , seg_count);
        if (idx > attr->cmap.glyph_id_array_count) {
            ...
            HPDF_PTRACE((" HPDF_TTFontDef_GetGlyphid idx=%u unicode=0x%04X "
                        "id=%u\n", idx, unicode, gid));
						printf(" HPDF_TTFontDef_GetGlyphid idx=%u unicode=0x%04X "
                        "id=%u\n", idx, unicode, gid);
            return gid;
        }
    }
                              ...
                              HPDF_UINT16    unicode)
{
	...
	printf("hpdf_ttfontdef getchar width char = %X  = %d , gid = %d \n " , unicode, unicode, gid );
    ...
    if (gid >= attr->num_glyphs) {
        HPDF_PTRACE((" HPDF_TTFontDef_GetCharWidth WARNING gid > "
                    "num_glyphs %u > %u\n", gid, attr->num_glyphs));
	    printf(" HPDF_TTFontDef_GetCharWidth WARNING gid > "
                    "num_glyphs %u > %u\n", gid, attr->num_glyphs);
		printf("warning , returning missed width\n");
    ...
    advance_width = (HPDF_UINT)hmetrics.advance_width * 1000 /
			...
			printf("returning %X \n ", (HPDF_INT16)advance_width);
			printf("hmetrics advance width = %d ", hmetrics.advance_width);
...
ParseHmtx  (HPDF_FontDef  fontdef)
{
    ...
    for (i = 0; i < attr->num_h_metric; i++) {
        if ((ret = GetUINT16 (attr->stream, &pmetric->advance_width)) !=
            ...
            return ret;
			if ( i < 10 ) 
				printf(" htmx i = %d , width = %d \n ", i, pmetric->advance_width);
                              ...
                              HPDF_Stream    stream)
{
    ...
    for (i = 0; i < HPDF_REQUIRED_TAGS_COUNT; i++) {
		...
		printf( "new table , table name %s  , length = %d  new_offset = %d \n" , REQUIRED_TAGS[i], length, new_offset );
